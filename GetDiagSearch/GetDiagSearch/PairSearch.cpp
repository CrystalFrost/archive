...
void PairSearch::Initialize(char* start, char* result, char* checkpoint, char* temp)
{
	fstream startFile;
	...
	/*
	// Задаём настройки генератора
	squareAGenerator.Initialize(genStart, genResult, genCheckpoint, genTemp);

	// Считываем собственные настройки
	Generator::Initialize(startParametersFileName, resultFileName, checkpointFileName, tempCheckpointFileName);

	// Сохраняем копию первоначальных настроек для возобновляемого поиска пары к очередному ДЛК
	initialState = (Generator)(*this);
	*/
}
...
void PairSearch::CreateCheckpoint()
{
	...
	if (checkpointFile.is_open())
	{
		...
		rename(tempCheckpointFileName, checkpointFileName);
	}
}
