...
int main()
{
	...
	// Очищаем массив использованных комбинаций пар элементов квадратов
	for (int i = 0; i < rank; i++)
	{
		for (int j = 0; j < rank; j++)
		{
	...
	while (!stop)
	{
		...
		for (int i = 0; i < rank; i++)
		{
			...
			if (columns[layerId][i][columnId] == 1 && rows[layerId][rowId][i] == 1 && cellsHistory[layerId][rowId][columnId][i] == 1)
			{
				// Обработка найденного значения
					...
					if (layerId == 1)
					{
						// Проверка получающейся пары на совместимость с другими парами квадратов
						if (pairsDictionary[square[0][rowId][columnId]][i] == 1)
						{
							cellValue = i;
							isGet = 1;
		...
		if (isGet)
		{
			...
			if (layerId == height - 1)
			{
				// Обрабатываем найденную пару, уже записанную в квадрат
					// Отмечаем использованние новой пары
					pairsDictionary[square[0][rowId][columnId]][square[1][rowId][columnId]] = 0;
					// Снимаем отметку с использования предыдущй пары
					if (oldCellValue != -1)
					{
						pairsDictionary[square[0][rowId][columnId]][oldCellValue] = 1;
					}
			}
			...
			if (rowId == rank - 1 && columnId == rank - 1 && layerId == height - 1)
			{
				...
				// Вывод заголовка с информацией о паре
				resultFile << "------------------------" << endl;
				resultFile << "Pair # " << squaresCount << endl;
				resultFile << "Square A: " << (squareDiagonal[0] == 1 ? "Diagonal" : (squareLatin[0] == 1 ? "Latin" : "???" )) << endl;
				resultFile << "Square B: " << (squareDiagonal[1] == 1 ? "Diagonal" : (squareLatin[1] == 1 ? "Latin" : "???" )) << endl;
				resultFile << "------------------------" << endl;

				cout << "------------------------" << endl;
				cout << "Pair # " << squaresCount << endl;
				cout << "Square A: " << (squareDiagonal[0] == 1 ? "Diagonal" : (squareLatin[0] == 1 ? "Latin" : "???" )) << endl;
				cout << "Square B: " << (squareDiagonal[1] == 1 ? "Diagonal" : (squareLatin[1] == 1 ? "Latin" : "???" )) << endl;
				cout << "------------------------" << endl;

				// Вывод квадратов пары
				for (int h = 0; h < height; h++)
				{
					for (int i = 0; i < rank; i++)
					{
		...
		{
			// Обработка факта ненахождения нового значения в ячейке (layerId; rowId; columnId)
				// Считывание текущего значения
				...
				if (layerId == height - 1)
				{
					if (cellValue != -1)
					{
						pairsDictionary[square[0][rowId][columnId]][cellValue] = 1;
					}
				}
